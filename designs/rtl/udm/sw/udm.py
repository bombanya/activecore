...
class udm:
    ...
    def wrelf32(self, base_offset, filename):
        ...
        try:
            ...
            e_version = f.read(4)
            e_version = struct.unpack("I", e_version)
            
            e_entry = f.read(4)
            e_entry = struct.unpack("I", e_entry)
            #print("-- e_entry: ", hex(e_entry[0]))
    
            e_phoff = f.read(4)
            e_phoff = struct.unpack("I", e_phoff)
            #print("-- e_phoff: ", hex(e_phoff[0]))
    
            e_shoff = f.read(4)
            e_shoff = struct.unpack("I", e_shoff)
            #print("-- e_shoff: ", hex(e_shoff[0]))
    
            e_flags = f.read(4)
            e_flags = struct.unpack("I", e_flags)
            ...
            for h in range(e_phnum[0]):
                prog_header = f.read(32)
                prog_header = struct.unpack("IIIIIIII", prog_header)
                PT_LOAD = 1
            ...
            for prog_header in prog_headers:
                ...
                f.seek(offset)
                dbs = f.read(size)
                dbs = struct.unpack('{}I'.format(len(dbs)>>2), dbs)
